...
if(!empty($_POST["toDate1"]) && !empty($_POST["fromDate1"]) && !empty($_POST["trade_confirmation"])) {
    ...
    $query = $dbh->prepare("
        SELECT a.cd_code, a.f_name, a.l_name, a.phone, a.bank_account, b.bank_short_name, a.ID, a.email, i.gst_register 
    ...
    <div class="col-lg-12">
      <div class="box-body table-responsive">';
        ...
        $query = $dbh->prepare("
              SELECT a.lot_size_execute, a.order_exe_price, a.order_date, a.side, b.symbol, a.dirty_price, a.accur_rate, a.ytm  
        ...
        <table class='table table-bordered'>
          ...
          <tbody>";
            foreach ($results as $res) {
              $total1 = $res['lot_size_execute'] * $res['dirty_price'];
              $total = $total + $total1;
              echo'
              <tr>
                <td>' . $i++ . '</td>
            ...
            $stmt = $dbh->prepare("
                  ...
                  AND b.user_name = ? 
            ");
            $stmt->execute([$cd_code, $fromDate, $toDate, $username]);
    ...
    <div class="row no-print">
      <div class="col-lg-12">
        &emsp;&emsp;<a href="loadReportPrint.php?cd='.$cd_code.'&toDate='.$toDate.'&fromDate='.$fromDate.'&tradeConfirmation=tradeConfirmation" target="_blank" class="btn btn-default">
...
{       
  ...
  $wc= $dbh->prepare("
      ...
      JOIN client_account c ON e.cd_code = c.cd_code
      JOIN symbol s ON e.symbol_id = s.symbol_id
      WHERE e.sub_user = :un AND e.order_date BETWEEN :fromDate AND :toDate
      ORDER BY e.order_date ASC
  ");
  ...
  <div class="col-lg-12">
    <div class="box-body table-responsive">';
      echo'<b>Summary of Trade</b><br> From : '.$fromDate.' - To : '.$toDate;
        $sql = $dbh->prepare("
            SELECT c.acc_type, o.occupation_name, c.tpn, c.f_name, c.l_name, c.ID, e.member_broker, e.cd_code,
        ...
        <div class='table-responsive' style='font-size: 10px;'>
          ...
          foreach($rows as $res1){
            $fullname = ($res1['acc_type'] === 'J') ? $res1['f_name'] : $res1['title'].'. '.$res1['f_name'].' '.$res1['l_name'];
            echo'
            <tr>
              <td>'.$res1['sub_user'].'</td>
              <td>'.$res1['cd_code'].'</td>
...
elseif (!empty($_GET['zge_export']))
{
    ...
    $columnHeader = '';  
    $i=1;
    $columnHeader = "SNO\t USER\t CD CODE\t TPN\t NAME\t CID\t BUY\t SELL\t PRICE\t AMOUNT\t SYMBOL\t DATE\t"; 
    $setData = '';  
    while ($rec=$stmt->fetch()){
      // $name = strtoupper($rec['title'].'. '.$rec['f_name'].' '.$rec['l_name']);
      $fullname = ($rec['acc_type'] === 'J') ? $rec['f_name'] : $rec['title'].'. '.$rec['f_name'].' '.$rec['l_name'];;
      if($stmt->rowCount() <= 0) 
      {}
      $rowData = '';  
      $value = $i++ . 
      ...
      "\t" .str_replace($search, $replace, $fullname).
      "\t".$rec['ID'].
      "\t".$rec['BUY'].
      "\t".$rec['SELL']). 
...
elseif(!empty($_POST["generate_gst_report"])) 
{
  ...
  <div class="col-lg-12">
    <div class="box-body table-responsive">';
      $query = $dbh->prepare("
            SELECT b.remarks, b.cd_code, b.amount, b.finance_date, a.f_name, a.l_name, a.ID, a.phone
            FROM bbo_finance b
