...
$username=$_SESSION['sess_username'];
                ...
                function client_commission_multiple_brokers($cd_code,$username)
                {
                     include ('../../CONNECTIONS/db.php');
                     $pr = $dbh->prepare('SELECT b.ID,b.bro_comm_id,b.f_name,b.l_name,b.cd_code,c.rate from client_account b,bbo_commission c
                      where b.bro_comm_id=c.bro_comm_id and b.cd_code=:ac and substr(b.user_name,1,7)=:un');
                     $pr->bindParam(':ac',$cd_code);
                     $pr->bindParam(':un',$username);
                     $pr ->execute();
                     $value= $pr->fetch();
                ...
                function cash_total_rights($cd_code,$username)
                {
                     include ('../../CONNECTIONS/db.php');
                     $wc= $dbh->prepare("SELECT a.cd_code,sum(a.amount) as tot, b.cd_code,b.ID
                        from rights_finance a,client_account b
                        where  a.cd_code=b.cd_code and a.cd_code=:ac and a.user_name=:un");
                     $wc->bindParam(':ac',$cd_code);
                     $wc->bindParam(':un',$username);
                     $wc ->execute();
                     $value= $wc->fetch();
                ...
                function cash_total_bond($cd_code,$username)
                {
                     include ('../../CONNECTIONS/db.php');
                     $wc= $dbh->prepare("SELECT a.cd_code,sum(a.amount) as tot, b.cd_code,b.ID
                        from bond_finance a,client_account b
                        where  a.cd_code=b.cd_code and a.cd_code=:ac and a.user_name=:un");
                     $wc->bindParam(':ac',$cd_code);
                     $wc->bindParam(':un',$username);
                     $wc ->execute();
                ...
                function cash_total_ipo($cd_code, $username)
                {
                     include ('../../CONNECTIONS/db.php');
                     $wc= $dbh->prepare("SELECT a.cd_code, sum(a.amount) as tot, b.cd_code, b.ID
                        FROM ipo_finance a, client_account b
                        WHERE a.cd_code=b.cd_code and a.cd_code=:ac and a.user_name=:un");
                     $wc->bindParam(':ac',$cd_code);
                     $wc->bindParam(':un',$username);
                     $wc ->execute();
                ...
                function check_orders($cdcode,$sy_id,$side,$p_code)
                {
                     ...
                     $wc= $dbh->prepare("SELECT * from orders where cd_code=:c and symbol_id=:sy_id and participant_code=:p");
                ...
                function order_audit($cdcode,$p_code,$u_name,$vol,$vol,$sy_id,$price,$side,$commis_amt,$flag_id,$u_name){
                    include ('../../CONNECTIONS/db.php'); 
                    ...
                    VALUES ('$cdcode','$p_code','$u_name','$vol','$vol','$sy_id','$price','$side','$commis_amt','$flag_id','$u_name')");
                     $b_order ->execute();
                }
                function order_auditTE($cdcode,$p_codeo,$u_name,$vol,$vol,$sy_id,$price,$side,$commis_amt,$flag_id,$p_code){
                    include ('../../CONNECTIONS/db.php'); 
                    $b_order = $dbh->prepare("INSERT into orders_audit(cd_code,participant_code,order_entry,buy_vol,order_size,symbol_id,price,side,commis_amt,flag_id,member_broker) 
                    VALUES ('$cdcode','$p_codeo','$u_name','$vol','$vol','$sy_id','$price','$side','$commis_amt','$flag_id','$p_code')");
                     if($b_order ->execute())
                     {
                        return 1;
                     }
                ...
                {               
                     include ('../../CONNECTIONS/db.php');
                     $bal=$dbh->prepare("SELECT sum(order_size) as sum from rights_issue WHERE ((type='S' and cd_code=:cd) OR (type='R' and cd_code=:cd) OR
                        (type='O' and cd_code=:cd)) and status=0");
                     $bal->bindParam(':cd',$cd);
                     $bal->execute();                     
                     $val=$bal->fetch();
                     $sum=$val['sum'];
                ...
                {               
                     include ('../../CONNECTIONS/db.php');
                     $bal=$dbh->prepare("SELECT sum(total_amount) as sum from rights_issue WHERE ((type='R' and renounce_cd_code=:cd and user_name=:un) OR (type='S' and cd_code=:cd and user_name=:un) OR 
                        (type='B' and cd_code=:cd and user_name=:un)) and status=0");
                     $bal->bindParam(':cd',$cd);
                     $bal->bindParam(':un',$username);
                     $bal->execute();                     
                     $val=$bal->fetch();
                     $sum=$val['sum'];
                     return $sum; 
                ...
                {               
                     include ('../../CONNECTIONS/db.php');
                     $bal=$dbh->prepare("SELECT sum(total_amount) as sum from rights_issue WHERE (type='R' and renounce_cd_code=:rcd and status=0) OR (type='S' and cd_code=:rcd and status=0) OR
                        (type='B' and cd_code=:rcd and status=0) ");
                     $bal->bindParam(':cd',$cd);
                     $bal->bindParam(':rcd',$rcd);
                     $bal->execute();                     
                     $val=$bal->fetch();
                     $sum=$val['sum'];
                     return $sum; 
                ...
                {               
                     include ('../../CONNECTIONS/db.php');
                     $bal=$dbh->prepare("SELECT sum(total_amount) as sum from ipo WHERE type='IPO' and cd_code=:cd and status=0");
                     $bal->bindParam(':cd',$cd);
                     $bal->execute();                     
                     $val=$bal->fetch();
                     $sum=$val['sum'];
                ...
                {               
                     include ('../../CONNECTIONS/db.php');
                     $bal=$dbh->prepare("SELECT sum(total_amount) as sum from orders_ipo WHERE cd_code=:cd");
                     $bal->bindParam(':cd',$cd);
                     $bal->execute();                     
                     $val=$bal->fetch();
                     $sum=$val['sum'];
                ...
                {               
                     include ('../../CONNECTIONS/db.php');
                     $bal=$dbh->prepare("SELECT sum(total_amount) as sum from bond WHERE type='BOND' and cd_code=:cd and status=0");
                     $bal->bindParam(':cd',$cd);
                     $bal->execute();                     
                     $val=$bal->fetch();
                     $sum=$val['sum'];
