...
/*$q222 = $dbh->prepare('SELECT a.*, b.institution_id, c.rate 
    from orders a, adm_participants b , bbo_commission c,client_account ca 
    ...
    if ($result) {
        ...
        if ($side === $s) {
            ...
            /*$b_fin = $dbh->prepare("INSERT INTO bbo_finance (cd_code,amount,user_name,remarks,flag,institution_id,flag_id,status) 
                VALUES ('$cd_code','$new_exe_amt','$member_broker','$remarks_sell','$flag_sell','$institution_id','$oidd','$finsellstatus')");
            $b_fin->execute();*/
        ...
        elseif ($side === $b) {
            ...
            if ($institution_id == $ins_id_mcams) {
                ...
                /*$sql = "INSERT INTO mcams_wallet (cd_code, amount, type, paid_to_user, trx_time)
                        VALUES 
                            ('$cd_code', '-$amt', 'DR', 'COMMISSION', CURRENT_TIMESTAMP),
...
if ($currentHour == 15) {
    ...
    $specifieddate = $dbh->prepare('
        SELECT SUBSTRING(max(e.order_date),1,10) dat FROM executed_orders e WHERE e.side = "S" AND e.order_date LIKE "%' . $dateselect . '%"
    ');
