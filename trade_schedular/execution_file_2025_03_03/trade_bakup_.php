...
//check trade schedular on or off
$check = $dbh->prepare("SELECT schedular_name, task_code, remarks, status FROM task_schedulars WHERE task_code='TS'");
$check->execute();
$row = $check->fetch();
$schedular_status = $row['status'];

if ($schedular_status == 'OFF') {
    echo'Trade Schedular OFF';
    echo'<br>'.$row['remarks'];
    die();
} else {
    echo'Trade Schedular ON';
}
...
$specifieddate = $dbh->prepare('SELECT SUBSTRING(max(e.order_date),1,16) dat from executed_orders e where e.side="S" and e.order_date  like "%'.$dateselect.'%"');
...
if(is_null($conditiondate) == 0){
 ...
 echo "TRADE";
  $get_symbol_id=$dbh->prepare('SELECT w.symbol_id,sum(w.lot_size_execute) s,w.order_date,s.paid_up_shares from executed_orders w 
left join symbol s on s.symbol_id=w.symbol_id
where order_date and w.order_date
like "%'.$conditiondate.'%" and w.side="S" group by w.symbol_id order by w.order_date ASC');
  ...
  $get_symbol_id->execute();*/
  foreach($get_symbol_id as $result){ 
        $min_vol_required= floor(0.0041 * $result['paid_up_shares'] / 100);
