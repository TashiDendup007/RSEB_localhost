...
if($currentHour == 15){
  ...
  $specifieddate = $dbh->prepare('SELECT SUBSTRING(max(e.order_date),1,10) dat from executed_orders e where e.side="S" and e.order_date  like "%'.$dateselect.'%"');
    ...
    // echo "<b>".$result['s'].'-->'.$result['symbol_id'].'->'.$min_vol_required."</b><br/>";
      ...
      if(floatval($result['s']) >= $min_vol_required)
      {
        $get_price=$dbh->prepare('SELECT 
                                  ...
                                  WHERE 
                                      side = "S" AND
                                      symbol_id = :symbol_id AND order_date like "%'.$conditiondate.'%" order by  order_date ASC');
        ...
        if($get_mp->rowcount() <= 0)
        {
          ...
          $up_insert=$dbh->prepare("INSERT into market_price (symbol_id,market_price) VALUES ('$symid','$avg_price')");
          $up_insert->execute();
        }
        else
        {
