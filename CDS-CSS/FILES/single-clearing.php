      ...
      <section class="content">
        ...
        <div class="box">
          <div class="box-header with-border">
            <h4 class="box-title">Clearing Details [<span style="font-size: 14px; color: red;">Click on the date to expand or collapse the list</span>], <i class="fa fa-times" style="color: #d10000;"></i>  Unsettled <i class="fa fa-check" style="color: #00b900;"></i> Settled</h4>
            ...
            <div class="row">
              <div class="col-lg-12">
                <div class="panel-group" id="accordion" role="tablist" aria-multiselectable="true">
                  <?php 
                    $stmt = $dbh->prepare("SELECT DATE(e.order_date) AS order_date FROM executed_orders e GROUP BY DATE(e.order_date) ORDER BY e.exe_id DESC LIMIT 15");
                    $stmt->execute();
                        ...
                        <div class="panel-heading" role="tab" id="heading_'.$i.'">
                          <h4 class="panel-title">
                            <a role="button" data-toggle="collapse" data-parent="#accordion" href="#collapse_id_'.$i.'" aria-expanded="true" aria-controls="collapse_id_'.$i.'">
                              ...
                              $check_status = $dbh->prepare("SELECT COUNT(*) AS present FROM executed_orders a WHERE a.status != 1 AND DATE(a.order_date) = ?");
...
<script type="text/javascript">
  ...
  function fetch_trade_details(exe_date) {
    showLoading();
    var op = 'get__trade__details';
    $.ajax({
        type: "POST",
        url: "rseb-sett-script.php",
        data: {
            get__trade__details: op,
            exe__date: exe_date
        },
        success: function(data) {
            hideloading();
  ...
  function sett_process() {
    var SETT = document.getElementById('sett').value;
    var date = document.getElementById('tdate').value;

    if (confirm("Are you sure you want to process the settlement ?")) {
      showLoading();
      $.ajax({
        type: "POST",
        url: "rseb-sett-script.php",
        data:'SETT='+SETT+'&date='+date,
        success: function(data){
          hideloading();
          $("#message").html(data);
          showMessage();
  ...
  $("#click").click( function () { 
    var tdate = $("#tdate").val();
    var dataString = 'load_clearing_data='+ tdate;
    
    if(tdate == ''){
      alert("Please Enter the Trade Date to load the data !");
    } else {
      $.ajax({
        type: "POST",
        url: "rseb-sett-script.php",
