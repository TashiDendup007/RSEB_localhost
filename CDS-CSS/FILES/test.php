...
$st=1;
        ...
        $q->bindParam(':symbol_id', $symbol_id);
        $q->execute();
          foreach($q as $row){
          ...
          $cds_holding_id=$row['cds_holding_id'];
            if($l_check >0 && $pending_in_vol>0){
              ...
              $l_check=$l_check-$pending_in_vol;
                ...
                $q->bindParam(':pending_in_vol_new', $pending_in_vol_new);
                $q->bindParam(':vol_new', $vol_new);
                $q->bindParam(':cd_code', $cd_code);
                $q->bindParam(':ccid', $cds_holding_id);
                $q->bindParam(':symbol_id', $symbol_id);
              if($q->execute()){
...
volume=:vol_new where cd_code=:cd_code and symbol_id=:symbol_id and cds_holding_id=:ccid");
$q->bindParam(':pending_out_vol_new', $pending_out_vol_new);
$q->bindParam(':vol_new', $vol_new);
$q->bindParam(':cd_code', $cd_code);
$q->bindParam(':ccid', $cds_holding_id);
$q->bindParam(':symbol_id', $symbol_id);
