    ...
    <section class="content">
      ...
      <div class="box"><br>
        ...
        <div class="col-xs-3">
          ...
          <?php
              $cn= $dbh->prepare("SELECT DISTINCT company_name from unclaimed_dividend");
              $cn->execute();
              echo'<select class="form-control" name="company_name" id="company_name" required="">';
              echo '<option value="ALL">-SELECT-</option>';
              while($res= $cn->fetch())
              {
                echo '<option value="'.$res['company_name'].'">'.$res['company_name'].'</option>';
              }
              echo'</select>';
            ?>
        </div>
...
<script type="text/javascript">
  ...
  $('#escorDetails').click(function() {
    ...
    $.ajax({
        type: "POST",
        url: "load.php",
        data: { company_name: company_name, escorReport: op },
