    ...
    <body onload="window.print();">
      <div class="wrapper">
        <section class="invoice" style="background:rgb(248, 249, 249);">
          ...
          $query= $dbh->prepare('SELECT a.*,b.symbol from executed_orders a,symbol b where  a.symbol_id=b.symbol_id and cd_code=:cd and :fromDate <= a.order_date and a.order_date <= :toDate');
            ...
            <tbody>";
              foreach($query as $res){
                ...
                <tr>
                  ...
                  <td>Nu. '.number_format($res['lot_size_execute']*$res['order_exe_price'],2,".",",").'</td>
                </tr>';
                if($res['side']=='B'){$totalb=$totalb+$total1;}if($res['side']=='S'){$totals=$totals+$total1;}
              }
              /*$totals=trade_confirm_sell($cd_code,$toDate,$fromDate);
              $totalb=trade_confirm_buy($cd_code,$toDate,$fromDate);*/
      ...
      <title>Commission Report</title>
        ...
        <section class="invoice" style="background:rgb(248, 249, 249);">
            <div class="row">
             ...
             $query= $dbh->prepare("SELECT e.order_date,c.title,c.f_name,c.l_name,c.ID,e.cd_code,s.symbol,e.lot_size_execute,e.order_exe_price,e.lot_size_execute* e.order_exe_price as amount from 
                  executed_orders e, client_account c, symbol s where e.cd_code=c.cd_code and e.symbol_id=s.symbol_id and :fromDate <= e.order_date and e.order_date <= :toDate 
                  and e.side='B' and member_broker=:un order by e.order_date ASC");
                $query->bindParam(':fromDate',$fromDate);
                $query->bindParam(':toDate',$toDate);
                $query->bindParam(':un',$username);
                $query->execute();
