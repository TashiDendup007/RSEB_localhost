       ...
       elseif($vb<$vs && $vb==$diff){
          ...
          for($x=$diff_chk; $x>0;){
                     ...
                     foreach ($pr as $value){   
                      ...
                      $pr = $dbh->prepare('SELECT * from orders WHERE price >=:op and sym_id=:sym_id and buy_vol >0 and side="B" ORDER BY buy_vol DESC');
                      $pr ->bindParam(':op',$op);$pr ->bindParam(':sym_id',$sym_id);
                      $pr ->execute();
                      foreach($pr as $value) {   
                               $oiddb=$value['order_id'];
...
$q222 = $dbh->prepare('SELECT a.*,b.institution_id from orders a, adm_participants b where a.participant_code=b.participant_code and exe_vol > 0');
$q222 ->execute();
foreach ($q222 as  $value){    
           $oidd=$value['order_id'];
           $price=$value['price'];
