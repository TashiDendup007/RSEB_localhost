...
elseif(!empty($_POST["cd_load_cli"])) {
  ...
  $wc = $dbh->prepare("SELECT 
        a.f_name, a.l_name, a.ID, a.phone, a.email, a.title
      FROM client_account a WHERE a.cd_code =:cd");
  $wc->bindParam(':cd', $cd);
  $wc->execute();
  $res = $wc->fetch();
  ...
  $i = 1;
  $wc= $dbh->prepare("SELECT s.symbol, a.symbol_id, a.volume, a.pledge_volume, a.block_volume, a.pending_in_vol, a.pending_out_vol, 
      (a.volume + a.pledge_volume + a.block_volume + a.pending_in_vol + a.pending_out_vol) AS total_vol
    FROM cds_holding a
      ...
      <section class="invoice" style="background:rgb(248, 249, 249);">
          ...
          foreach($all_executed_orders as $res1){
            ...
            $list_ord= $dbh->prepare('SELECT sum(lot_size_execute) as totlot , cast(avg(order_exe_price) as decimal(13,2)) as avgp 
              FROM executed_orders  where participant_code=:pc and symbol_id=:syid and side="B" and  order_date >= :fdate  and order_date <= :tdate');
            $list_ord->bindParam(':pc',$participant_code);
            $list_ord->bindParam(':syid',$res1['symbol_id']);
            $list_ord->bindParam(':fdate',$fromDate);
            $list_ord->bindParam(':tdate',$toDate);
...
elseif (isset($_POST['get_pending_order_list'])) {
  ...
  <table id="example1" class="table table-bordered table-striped" width="100%">
    ...
    <tbody>';
      $wc = $dbh->prepare("
          SELECT a.order_id, a.cd_code, a.participant_code, a.member_broker, a.order_size, a.order_entry, a.flag_id, a.sell_vol, a.buy_vol, a.price, a.side, a.commis_amt, a.order_date, b.symbol, b.symbol_id 
          FROM symbol b
...
<script type="text/javascript">
  ...
  $("#sellsubmit").click( function() { 
    ...
    var dataString = 'cdcode='+cdcode+'&p_code='+p_code+'&u_name='+u_name+'&vol='+vol+'&sy_id='+sy_id+'&price='+price+'&side_for_order='+side+'&b_commis='+b_commis+'&avl_vol='+avl_vol+'&pov='+pov+'&piv='+piv;
