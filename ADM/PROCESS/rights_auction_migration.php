<?php
	...
	try {
	    ...
	    $log_file = __DIR__ . '/migration_auction_log_' . date('Ymd_His') . '.log';
		$log_messages = '';
	    ...
	    $orders_stmt = $dbh->prepare("
	        ...
	        AND r.status = 1 
	        AND r.type IN ('SA', 'B') 
			AND r.allocated_size > 0 
			AND r.bid_price >= 30.1
	    ...
	    $status_update_stmt = $dbh->prepare("
	        UPDATE rights_issue SET status = 2 WHERE symbol_id = ? AND order_id = ?
	    ");
	    ...
	    foreach ($orders as $order) {
	        ...
	        $username = str_starts_with($order['user_name'], 'MEM') ? $order['user_name'] : 'EMPRSEB009';
	        $institution_id = isset($order['institution_id']) ? $order['institution_id'] : 1;
	        $remarks = "Record via Rights Share Auction: {$allocated_vol} shares";
