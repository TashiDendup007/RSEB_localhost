...
foreach($get_symbol_id as $result){   
  $get_price=$dbh->prepare('SELECT order_exe_price from executed_orders where side="S" and symbol_id=:symbol_id and order_date like "%'.$dateselect.'%" order by  order_date ASC');
  $get_price->bindParam(':symbol_id',$result['symbol_id']); 
  $get_price->execute();
  $price=$get_price->fetch();
  $up_price=$dbh->prepare('UPDATE market_price SET market_price=:close_price where symbol_id=:symbol_id');
