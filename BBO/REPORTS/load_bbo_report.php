<?php
  ...
  if(!empty($_POST["toDate1"]) && !empty($_POST["fromDate1"]) && !empty($_POST["trade_confirmation"])) {
    ...
    <div class="col-lg-12">
      <div class="box-body table-responsive">';
        $query = $dbh->prepare("SELECT 
            a.cd_code, a.f_name, a.l_name, a.phone, a.bank_account, b.bank_short_name, a.ID, a.email, i.gst_register 
        ...
        <table class='table table-bordered'>
          ...
          <tbody>";
            ...
            if ($sec_type === 'OS') {
              ...
              ");
              $stmt->execute([$cd_code, $fromDate, $toDate, $mem_code]);
    ...
    <div class="row no-print">
      <div class="col-lg-12">
        &emsp;&emsp;<a href="loadReportPrint.php?cd='.$cd_code.'&toDate='.$toDate.'&fromDate='.$fromDate.'&sec_type='.$sec_type.'&tradeConfirmation=tradeConfirmation" target="_blank" class="btn btn-default"><i class="fa fa-print"></i> Print</a>
      </div>';
        echo'
        <div class="row" style="margin-top: 5px;">
          <div class="col-lg-8">
            &emsp;&emsp;&emsp;
            The email will be sent to : <b>'.$row['email'].'</b>. <br>&emsp;&emsp;&emsp;
            <small style="color: red;">
              If this is incorrect, Please update it in account registration and then send the email.
            </small>
          </div>
          <div class="col-lg-4 float-left">
            &emsp;&emsp;
            <button class="btn btn-primary" onclick="sendMailNRB(\''.$cd_code.'\', \''.$fromDate.'\', \''.$toDate.'\', \''.$sec_type.'\')"><i class="fa fa-envelope"></i> Send Mail</button>
          </div>
        </div>

        <script type="text/javascript">
          function sendMailNRB(cd_code, from_date, to_date, sec_type) {
            if (confirm("Do you want to continue?")) {
              ...
              $.ajax({
                ...
                data: "cd_code=" + cd_code + "&from_date=" + from_date + "&to_date=" + to_date + "&sendNRBReportViaMail=" + op + "&sec_type=" + sec_type,
  ...
  elseif (isset($_POST['sendNRBReportViaMail']) && isset($_POST['cd_code'])) {
      ...
      // GET EMAIL
      $sql = $dbh->prepare("SELECT a.cd_code, a.ID, a.title, a.f_name, a.l_name, a.email, a.phone, a.tpn, a.address, i.name, a.bank_account, b.bank_short_name, i.gst_register 
        FROM client_account a 
        LEFT JOIN banks b ON a.bank_id = b.bank_id 
        LEFT JOIN adm_institution i ON a.institution_id = i.institution_id
        WHERE a.cd_code = ?
...
elseif(!empty($_POST["toDate1"]) && !empty($_POST["fromDate1"]) && !empty($_POST["cashtrnx"])) 
{
    ...
    <div class="col-lg-12">
      <div class="box-body table-responsive">';
        $query = $dbh->prepare("
            ...
            WHERE e.member_broker = :un 
            AND e.side = 'B' 
            AND e.order_date BETWEEN :fromDate AND :toDate
    ...
    <div class="row no-print">
      <div class="col-lg-12">
      &emsp;&emsp;<a href="loadReportPrint.php?toDate='.$toDate.'&fromDate='.$fromDate.'&cashtrnx=cashtrnx&sec_type='.$sec_type.'" target="_blank" class="btn btn-default"><i class="fa fa-print"></i> Print</a>
      &emsp;&emsp;<a href="load_bbo_report.php?ge_export=ge_export&fromDate='.$fromDate.'&toDate='.$toDate.'&sec_type='.$sec_type.'" class="btn btn-success"><i class="fa fa-save"></i> Export</a>
      </div>
    </div>
    <br>';
  }
  ...
  {       
      ...
      while ($rec=$wc->fetch()){
        ...
        "\t". str_replace($search,$replace,$rec['lot_size_execute']) . 
  ...
  elseif(!empty($_POST["toDate1"]) && !empty($_POST["fromDate1"]) && !empty($_POST["dTradeDetails"])) {
      $toDate = $_POST['toDate1'].' 23:59:00';
      $fromDate = $_POST['fromDate1'].' 00:00:00';
      $sec_type = $_POST['sec_type'];
      ...
      $sql = $dbh->prepare("
        SELECT o.occupation_name, c.acc_type, c.tpn, c.f_name, c.l_name, c.ID, e.member_broker, e.cd_code,
      ...
      <div class='col-lg-12'>
        <div class='box-body table-responsive'>
          <b>Summary of Trade</b><br> From : ".$fromDate." - To : ".$toDate."
          <div class='table-responsive'>
            ...
            foreach($rows as $res1) {
              $fll_name = ($res1['acc_type'] === 'I') ? $res1['title'].'. '.$res1['f_name'].' '.$res1['l_name'] : $res1['f_name'];
              echo'
              <tr>
                <td>'.$res1['sub_user'].'</td>
                <td>'.$res1['cd_code'].'</td>
                ...
                <td>'.number_format($res1['amount'], 2).'</td>
    ...
    <div class="row no-print">
      <div class="col-lg-12">
        &emsp;&emsp;<a href="load_bbo_report.php?zge_export=zge_export&fromDate='.$fromDate.'&toDate='.$toDate.'&sec_type='.$sec_type.'" class="btn btn-success"><i class="fa fa-save"></i> Export</a>
      </div>
    </div>
    <br>';
  }
  elseif (!empty($_GET['zge_export']))
  {
    ...
    $executed_orders = $dbh->prepare("
            SELECT z.DzongkhagName, c.DzongkhagID, o.occupation_name, c.acc_type, c.tpn, c.f_name, c.l_name, c.ID, e.member_broker, e.cd_code,
              COALESCE(IF(e.side = 'B', e.lot_size_execute, NULL), 0) AS BUY,
              COALESCE(IF(e.side = 'S', e.lot_size_execute, NULL), 0) AS SELL,
              e.order_exe_price, e.lot_size_execute * e.order_exe_price AS amount, s.symbol, e.order_date, e.sub_user, c.title 
            FROM 
    ...
    $columnHeader = '';  
    $i=1;
    $columnHeader = "SNO\t USER\t CD CODE\t TPN\t NAME\t CID\t SYMBOL\t BUY\t SELL\t PRICE\t AMOUNT\t DATE\t OCCUPATION\t DZONGKHAG\t"; 
    $setData = '';  
    while ($rec=$executed_orders->fetch()){
      $name = ($rec['acc_type'] === 'I') ? strtoupper($rec['title'].'. '.$rec['f_name'].' '.$rec['l_name']) : $rec['f_name'];
  ...
  elseif(!empty($_POST["generate_gst_report"])) 
  {
    $fromDate = $_POST['fromDate1'].' 00:00:00';
    $toDate = $_POST['toDate1'].' 23:59:00';
    ...
    <div class="col-lg-12">
      <div class="box-body table-responsive">';
        $query = $dbh->prepare("SELECT 
            b.remarks, b.cd_code, b.amount, b.finance_date, a.f_name, a.l_name, a.ID, a.phone
            FROM bbo_finance b
            LEFT JOIN client_account a ON b.cd_code = a.cd_code
            WHERE b.flag = 5
            AND b.finance_date BETWEEN ? AND ? 
            AND SUBSTRING(b.user_name, 1, 7) = ?
        ");
        $query->execute([$fromDate, $toDate, $part_code]);
        $rows = $query->fetchAll(PDO::FETCH_ASSOC);
        
        echo"
        <p class='center'>GST Transaction <br> From :  " . $fromDate . " - To : " . $fromDate . "</p>

        <table class='table table-bordered' id='gst_report_table'>
          <thead>
            <tr style='background-color:#333;color:#fff'>
              <th>#</th>
              <th>CD Code</th>
    ...
    <div class="row no-print">
      ...
      <div class="col-lg-6 text-right">
        <a href="load_bbo_report.php?export_gst_report=export_gst_report&fromDate=' . $fromDate . '&toDate=' . $toDate . '&part_code=' . $part_code . '" class="btn btn-success"><i class="fa fa-file"></i> Export</a>
        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
      </div>
    </div>
    <br>
    <script type="type/javascript">
      $("#gst_report_table").DataTable({
         "lengthMenu": [[10, 20, 50, -1], [10, 20, 50, "All"]]
      });
    </script>';
    exit();
  }
  elseif (!empty($_GET['export_gst_report']))
  {
      $replace = array("\n","\r\n","\r");
      $search  = array('','','');
      ...
      $query = $dbh->prepare("
          SELECT 
              b.remarks, b.cd_code, b.amount, b.finance_date, 
              a.f_name, a.l_name, a.ID, a.phone
          FROM bbo_finance b
          LEFT JOIN client_account a ON b.cd_code = a.cd_code
          WHERE b.flag = 5
          AND b.finance_date BETWEEN ? AND ? 
      ...
      $columnHeader = "SNO\tCD CODE\tAmount\tRemarks\tDate\t";
      $setData = '';
      $i = 1;
      ...
      while ($rec = $query->fetch(PDO::FETCH_ASSOC)) {
          $rowData = $i++ . "\t" .
              ...
              str_replace($search, $replace, $rec['finance_date']) . "\t";
