        ...
        <div class="row">
          <div class="col-xs-12 table-responsive">
            <table class="table  table-striped">
              ...
              $i = 1;
              foreach($wc as $state) {                       
                if($state['type'] == 'S'){ $side='SUBSCRIBE'; }elseif ( $state['type']=='R'){ $side='RENOUNCE';}else{ $side='BID';}
...
elseif(!empty($_GET['ge_export'])) {
    ...
    $wc= $dbh->prepare("SELECT * FROM rights_issue WHERE order_date BETWEEN :fdate AND :tdate ");
    $wc->bindParam(':fdate',$fromDate);
    $wc->bindParam(':tdate',$toDate);
    $wc->execute(); 
    ...
    while ($rec=$wc->fetch()) { 
      ...
      $p = ($rec['type'] == 'B') ? $rec['bid_price'] : $rec['face_value'];
      $totala = ($rec['type'] == 'B') ? $rec['bid_price'] * $rec['order_size'] : $rec['total_amount'];
...
elseif (isset($_POST["ipoaudit"])) {
    ...
    $wc = $dbh->prepare("SELECT i.bid_price, i.order_size, i.type, i.cd_code, i.user_name, i.order_date, c.ID, c.f_name, c.l_name 
      FROM ipo i
    ...
    <section class="invoice" style="background:rgb(248, 249, 249);">
      ...
      <div class="row">
        <div class="col-lg-12">
          <div class="lead" style="font-size: 70%; margin-top:-10px;"><strong>FROM : '.$fromDate.' TO : '.$toDate.'</strong></div>
        </div>
      </div>
      <div class="row">
