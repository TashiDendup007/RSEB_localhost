            ...
            <div class="box">
              <div class="box-body">
                <div class="col-lg-6 col-sm-12">
                  ...
                  <select name="sec_type" id="sec_type" class="form-control" onChange="get__order__list(this.value);">
...
<script>
  ...
  function get__order__list(val) {
    if (val == '') {
       $('#table_pending_orders').html('');
    } else {
      $.ajax({
        type: "POST",
        url: "load.php",
        data:'get_pending_order_list=get_pending_order_list&sec_type=' + val + '&usernmae=<?php echo addslashes($username); ?>',
        cache: false,
  ...
  function fun(i) {
    ...
    var sec_type = document.getElementById('sec_type').value;

    var regex = /^\d+(\.\d{1,2})?$/;
    if (!regex.test(e_p)) {
        hideloading();
        alert("Please enter a valid price with at most 2 decimal places.");
        return false;
    }
        ...
        success: function(response) {
          ...
          if (response.status === 'success') {
            ...
            $.ajax({
              type: "POST",
              url: "load.php",
              data:'get_pending_order_list=get_pending_order_list&sec_type=' + sec_type + '&usernmae=<?php echo addslashes($username); ?>',
              cache: false,
              success: function(data){
                hideloading();
                $('#table_pending_orders').html(data);
