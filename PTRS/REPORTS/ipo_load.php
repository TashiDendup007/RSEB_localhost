...
else if(isset($_POST["ipoaudit"])) 
{
    $toDate = $_POST['toDate'].' 23:59:00';
    $fromDate = $_POST['fromDate'].' 00:00:00'; 
    $wc= $dbh->prepare("SELECT * FROM ipo where :fdate <= order_date and order_date <= :tdate ");
    $wc->bindParam(':fdate',$fromDate);
    $wc->bindParam(':tdate',$toDate);
    $wc->execute();
    date_default_timezone_set("Asia/Thimphu");
...
{       
    ...
    /*<img src="../../dist/img/Logo.png"> &emsp; 
                  <b style="font-size: 130%;">ROYAL SECURITIES EXCHANGE OF BHUTAN<b>*/
    $columnHeader = "SNO" . "\t" . "TYPE" . "\t". "CD CODE"  . "\t". "ORDER VOLUME" . "\t". "PRICE" . "\t". "TOTAL AMOUNT" . "\t" . "USER NAME" . "\t" . "ORDER TIME" . "\t"; 
    $setData = '';  
    while ($rec=$wc->fetch()) { 
           if($wc->rowCount() <= 0) 
           {}
            ...
            $rowData = '';  
            $value = $i++ . "\t ". str_replace($search,$replace,$rec['type']) . "\t". str_replace($search,$replace,$rec['cd_code']). "\t". str_replace($search,$replace,$rec['order_size']) . 
             "\t". str_replace($search,$replace,$p) . "\t". str_replace($search,$replace,$totala)
